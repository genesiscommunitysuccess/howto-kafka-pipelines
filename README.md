...
The example uses two simple tables called `PRICE_PUBLISHED` and `PRICE_RECEIVED` which both contain a String primary key `ISIN` and additional Double field `BOND_PRICE` 
so that the record consumed from kafka can be easily serialised/deserialised into a database record for that table using kafka's provided String and Double serialiser/deserialiser classes respectively. 

Although we are using the serialisers/deserialisers from the kafka libraries, the Genesis framework allows you to embed a custom implementation into your project. An example of this
has been given in [PriceReceivedDeserialiser](server/kafka-pipelines-app/src/main/kotlin/global/genesis/kafka/PriceReceivedDeserialiser.kt) where we deserialize a Genesis DAO record.
For more information on how to create your own serialiser/deserialiser class please see the following: [Custom Serializers in Apache Kafka](https://www.baeldung.com/kafka-custom-serializer)

## Adding Kafka to your Genesis application
When needing to integrate with kafka in you Genesis application, there are a few set up steps which must be completed. Overviews for each step have been provided below and further
...
Then check the [Quick Start](https://learn.genesis.global/docs/getting-started/quick-start/) guide to start up the application itself.
